...
<%@ include file="/views/common/header.jsp" %>
	<section id="content">
		...
		<!-- jsp로 연결하지 말고 ajax 활용해서 구현하기 -->
		<div id="mbAllC">
			<button id="memberAll">전체 회원 조회</button><br>
			<input type="text" id="id"><button>아이디 조회</button>
		</div>
		...
		<style>
			...
			margin: 10px auto;}
			#id{margin-right:5px;}
			div#memberList>table{width:100%; margin:0 auto; border:1px solid black; border-collapse:collapse; clear:both; }
			div#memberList>table th, div#memberList>table td {border:1px solid; padding: 5px 0; text-align:center;}
		</style>
	</section>
	<script>
		...
		$("#id").next().click(e=>{
			...
			if(id!=""){
				$.ajax({
					url:"<%=request.getContextPath()%>/ajax/searchMember.do",
					data:{"userId":id},
					success:data=>{
						//console.log(data);
						const $table=$("<table>");
		...
		// 전체 회원 조회
		$("#memberAll").click(e=>{
			$.get("<%=request.getContextPath()%>/ajax/memberAll.do",data=>{
				//console.log(data);
				const $table=$("<table>");
				const $header="<tr><th>아이디</th><th>이름</th><th>성별</th><th>나이</th><th>주소</th><th>이메일</th><th>전화번호</th><th>취미</th><th>가입일</th></tr>";
				$table.html($header);
